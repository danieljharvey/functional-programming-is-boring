...
describe("Lesson 7 - parsers", () => {
  ...
  describe.skip("MSN parser", () => {
    ...
    it("Batch number success", () => {
      expect(runParser(batchNumberParser, "123456")).toEqual(some(123456));
    });
    it("ManufacturerCode fail", () => {
      expect(runParser(manufacturerParser, "!")).toEqual(none());
    });
    it("ManufacturerCode success", () => {
      expect(runParser(manufacturerParser, "H")).toEqual(some("Secure"));
    });
    it("PurchasingCompany fail", () => {
      expect(runParser(purchasingCompanyParser, "**!?")).toEqual(none());
    });
    it("PurchasingCompany success", () => {
      expect(runParser(purchasingCompanyParser, "OB")).toEqual(some("OB"));
    });
