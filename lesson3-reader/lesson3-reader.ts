...
const reader = <R,A>(runReader: (r: R) => A): Reader<R,A> => ({type:"Reader",runReader})

// take A and plop it into the Reader context
const pure = <R,A>(a: A): Reader<R,A> => ({ type: "Reader", runReader: _ => a })

// run the computation, by passing it the environment
const runReader = <R,A>(environment: R, value: Reader<R,A>): A => value.runReader(environment)
...
// Let's add mapping
const map = <R,A,B>(fn: (a: A) => B, reader: Reader<R,A>): Reader<R,B> => ({
  type: "Reader",
...
const bind = <R,A,B>(fn: (a:A) => Reader<R,B>, reader: Reader<R,A>): Reader<R,B> => ({
  type: "Reader",
  runReader: r => fn(reader.runReader(r)).runReader(r)
})

const horseNameExists = (horseName: string): Reader<HorseInformation,string> => 
   reader(horseInfo => 
